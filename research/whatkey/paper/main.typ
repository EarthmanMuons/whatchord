...
study a stricter interactive setting: a musician plays a MIDI keyboard, a chord
recognizer emits a causal stream of symbolic chord hypotheses, and the key
detector must update after each chord while abstaining when the evidence is
insufficient. We define an evaluation protocol for streaming key estimation with
abstention, including selective accuracy, coverage, stability, modulation lag,
and piece-level paired statistics, and evaluate a family of causal detectors on
four corpus-derived fixture sets. The final system is a filtered hidden Markov
model (HMM) over profile-correlation emissions, with one constrained symbolic
cue that redistributes evidence within same-tonic major/minor pairs. Three
results are central. First, the memory dial is a timescale selector, not a
noise-accuracy tradeoff: short memory fits local-key labels, while longer memory
fits section-key labels. Second, same-tonic mode confusions can be reduced
without disturbing tonic selection when symbolic chord-quality evidence is
confined to parallel-key pairs. Third, adaptive temporal alternatives trade
stability for faster key change detection and do not improve the selected
section-key operating point. On held-out pop-song fixtures, the causal
abstaining detector reaches statistical parity with standard offline music21 key
...
center the player is in right now, inferred from what they have played. This is
key finding, one of the oldest problems in music information retrieval, but the
...
blur, and wrong notes. There is no second pass and no lookahead; the Viterbi
decodings that dominate published HMM key-finding are unavailable by
construction.
...
Third, *abstention is a first-class outcome*. A modal vamp has no single right
answer, and asserting one confidently is worse than staying quiet. We therefore
measure coverage and accuracy-on-claimed as an inseparable pair, along with
stability, modulation lag, and time-to-first-claim, rather than accuracy alone.
...
+ *An explicit fixture-based data and reproducibility design*: chord streams are
  ...
  corpora are rebuilt locally rather than redistributed.
+ *A causal HMM specification for chord-recognition output*: filtered
  ...
  cue.
+ *The timescale finding*: the emission-memory dial selects which granularity of
  key structure a causal detector reports. Dose-response curves on two
  annotation cultures run in opposite directions with no interior optimum
  (@fig-dose), the crossover is significant on held-out data in both directions,
  and it reproduces within one corpus on identical performed input as a function
  of annotated segment length (@fig-segment). Accuracy comparisons between key
  detectors are therefore label-timescale dependent.
...
Those systems demonstrate the value of local-key estimation, but their usual
setting is offline score analysis. Our aim is narrower and more constrained:
causal filtering over chord-recognition events with abstention and stability
metrics.
...
are part of the input distribution.
...
leaderboards, which are audio-based on different data). Ground truth comes from
sources spanning two annotation cultures: When in Rome's analyst local keys and
RomanText annotations @wheninrome @tymo2019, the ASAP corpus of aligned
...
hold duration. After each event the detector outputs either a ranked list of
(key, confidence) hypotheses or an explicit abstention. It never sees the future
and is never re-run on edited history. The key space is the 24 major and minor
keys over pitch classes; enharmonic spelling is a presentation concern.

*Metrics.* The top-ranked claim is scored. Accuracy-like metrics are reported in
the selective-prediction frame: coverage (fraction of events with a claim) and
accuracy on claimed events form an inseparable pair, and abstentions are never
errors. Exact accuracy is complemented by the MIREX-weighted score (fifth 0.5,
relative 0.3, parallel 0.2) @mirex. Temporal behavior is measured by modulation
matching and lag (an annotated key change is matched when the detector claims
the new key before the next change; unmatched changes are censored counts, never
averaged into lag), spurious switches per piece (a switch is spurious only when
the annotation did not change and the new claim is not the annotated key), and
...
ground truth into the observations. Results are comparable only within a fixture
version, because fixture contents embed a specific version of the
...
capture code (pedal-aware sounding sets, debounce, minimum-duration commit), so
...
Multiplication in the last pseudocode line is elementwise; the transition matrix
supplies persistence over hidden keys, and the normalized posterior is the
filtered belief after the current event. In words, the profile scores are
...
temporal persistence over hidden keys. Reintroducing history both as an emission
blend and as state persistence double-counts evidence. Second, the emission
memory controls how local each observation is. The next section shows that this
...
keys and key signatures (Isophonics, ASAP) name the section key and absorb
...
development annotation scales gives the dose-response curves of @fig-dose. On
the local-key labels, accuracy falls essentially monotonically with memory and
modulation matching halves; on the section-key labels, accuracy climbs to a
broad plateau from 8 s outward while coverage and stability keep improving
...
Three controls close the alternative explanations. The crossover survives noise:
on the evaluation-only overlap corpus, identical performed recordings scored
...
#figure(
  ...
  caption: [The crossover within a single corpus on identical performed input
    ...
    granularity, reorder the two configurations.],
) <fig-segment>
...
*Ablations.* Two full 16-cell factorials (functional blend, progression blend,
duration weighting, recognizer-confidence weighting) ran on each annotation
scale. The functional blend adds key-relative chord-function points to the
profile score; the progression blend adds cadential-transition points; duration
weighting uses hold duration instead of one vote per event; and
...
the annotation scale: removing it is a significant paired win for section-key
annotations, while for local-key annotations it is load-bearing (+0.061 exact,
CI95 [+0.019, +0.108], p = 0.010). This is direct evidence that the functional
rules primarily detect brief excursions. The progression blend's earlier value
was architecture-specific and washes out under the HMM. Duration weighting helps
on both scales (+0.02 to +0.05). Recognizer-confidence weighting is inert in
five out of five paired tests, a useful negative result because exploiting
recognizer confidence was a natural hypothesis for this setting. The selected
emissions are therefore pure profile correlation, and a long-standing behavioral
failure (a 12-bar blues misread as its subdominant) vanished with the deleted
rules rather than through any added mechanism.
...
The useful constraint is mass preservation within a musically related pair: when
evidence is redistributed only between two hypotheses sharing an invariant, the
cue can help decide between those hypotheses without adding evidence to
unrelated keys.
...
12-dimensional correlation. But our observations are symbolic: when the played
chord is rooted on a candidate tonic with a home quality, its major or minor
quality is the most direct mode cue available. Applied as a log-odds tilt within
the parallel pair after the emission softmax, and then rescaled so the pair's
emission sum is unchanged, the cue cannot move tonic evidence at all. This
prevents the failure mode observed for less constrained tonic-bonus and
functional-rule variants, which could fight modulation tracking. Strength 2 was
selected after a 0.25-to-4 sweep with a 2-to-4 plateau: paired exact wins on
both annotation scales (Isophonics +0.016, p = 0.030; When in Rome +0.030, p =
...
the rival's tonic chord is ordinary diatonic harmony (the vi chord), not rare
mode mixture, so isolated chord quality fires constantly for the wrong twin. Two
sharpened cues were built and swept: a bass-gated tilt (fires only when the
chord's root is also its bass) and a cadential-bigram tilt (a dominant-quality
chord resolving down a fifth onto a home-quality chord). The cadence cue is
inert, echoing the progression ablation. The bass cue works mechanically
(relative confusion 6% to 4% of claims) but misses paired significance on the
primary scale (+0.007, p = 0.055), trends negative on the other (-0.009, p =
0.056), and doubles spurious p90; unlike the parallel tilt's broad plateau it
turns harmful at strength 2, the signature of a weak signal. Not adopted;
retained in code as a measured negative.
...
any chord-quality nudge between fifth neighbors is the already-refuted tonic
bonus reborn. Empirically the residual fifth errors also lack an exploitable
shape, splitting nearly evenly between the dominant and subdominant sides (57/43
and 56/44 on the two mode-resolved corpora).
...
building one we measured whether the system responds to the dwell parameter it
would refine: sweeping self-transition from 0.7 to 0.98 (implied mean dwell
roughly 3 to 50 events) moves every metric along the coverage-accuracy curve
rather than off it (Isophonics exact within 0.008, spurious p90 pinned at 1).
This probe does not prove that explicit-duration modeling is never useful for
key estimation, but it shows little headroom for the selected operating point.

*Bayesian online changepoint detection, built and measured.* The probe addresses
the changepoint prior (constant-hazard BOCPD implies the same geometric dwell)
but not BOCPD's distinct contribution, the run-length-adaptive evidence window:
pooling evidence back to the inferred section start instead of through a fixed
30 s decay. We built the constant-hazard detector for the 24-key space
@adams2007 with emission conventions identical to the selected HMM (including
the mode tilt), so differences isolate the window (@tab-bocpd).
...
versus 1). One finding worth keeping: at matched reactivity BOCPD dominates the
HMM's fast settings (0.765 exact at 135 modulations versus 0.736 at 141 for the
HMM at a 2 s half-life), so adaptive windowing has value for local-key tracking;
...
on the two mode-resolved splits, the music21 baselines on Isophonics with paired
and matched-coverage comparisons, and a descriptive mode-confusion breakdown.
The complete artifacts are committed with the project.
...
compare only against baselines runnable on our fixtures.
...
definition rather than a detail of the dataset.
