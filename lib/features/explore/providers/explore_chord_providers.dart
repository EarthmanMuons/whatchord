...
final exploreSeedChordIdentityProvider = Provider<ChordIdentity>((ref) {
  throw UnimplementedError('Explore chord seed must be overridden per route.');
});

final exploreChordProvider =
    NotifierProvider<ExploreChordNotifier, ExploreChordState>(
      ExploreChordNotifier.new,
    );

final exploreChordIdentityProvider = Provider<ChordIdentity>((ref) {
  final state = ref.watch(exploreChordProvider);
  return buildExploreChordIdentity(state);
});

final exploreChordMembersProvider = Provider<List<String>>((ref) {
  final identity = ref.watch(exploreChordIdentityProvider);
...
final exploreScaleDegreeProvider = Provider<ScaleDegree?>((ref) {
  final identity = ref.watch(exploreChordIdentityProvider);
  final tonality = ref.watch(selectedTonalityProvider);
  return tonality.scaleDegreeForChord(identity);
});

final exploreNormalizedVoicingProvider = Provider<List<int>>((ref) {
  final identity = ref.watch(exploreChordIdentityProvider);
  return normalizedVoicingForIdentity(identity);
});

final exploreAvailableExtensionOptionsProvider =
    Provider<List<ExploreExtensionOption>>((ref) {
      final state = ref.watch(exploreChordProvider);
      return buildExploreExtensionOptions(
        quality: state.quality,
        currentExtensions: state.extensions,
      );
    });

class ExploreChordNotifier extends Notifier<ExploreChordState> {
  @override
  ExploreChordState build() {
    final identity = ref.watch(exploreSeedChordIdentityProvider);
    return ExploreChordState.fromIdentity(identity);
  }

  void setRootPc(int rootPc) {
    final next = state.copyWith(rootPc: rootPc);
    state = _withValidBass(next);
  }

  void setQuality(ChordQualityToken quality) {
  ...
  void setExtensions(Set<ChordExtension> extensions) {
    final normalized = normalizeExtensionsForQuality(
      quality: state.quality,
      extensions: extensions,
    );
    final next = state.copyWith(extensions: normalized);
    state = _withValidBass(next);
  }

  void setBassPc(int bassPc) {
    final next = state.copyWith(bassPc: bassPc);
    state = _withValidBass(next);
  }
